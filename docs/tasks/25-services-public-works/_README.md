...
A hub linking to the city's services and public works in one place. A 2-column grid of large cards
covers: 🚧 Obras em Andamento (links to SCREEN 26), 📋 Solicitar Serviço (request a city service
like ILUMINAÇÃO, PODA), 🩺 UBS Mais Próximas, 🎓 Escolas, 🚌 Transporte, 📞 Telefones Úteis, etc.
Each card shows an emoji + label + short subtitle. Tapping each navigates to the appropriate
destination (SCREEN 26 for public works; future screens for the rest).

The screen is a **discovery surface** — citizens find what they need without searching menus.
...
- **Some cards link to external resources** (the prefecture's existing services portal); when
  possible, deep-link into specific sections.
