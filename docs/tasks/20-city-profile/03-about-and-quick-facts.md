...
- **About card**: a short descriptive paragraph about the city ("Cidade litorânea com 22.456
  habitantes e 92 km² divididos em 18 bairros...") — sourced from the city's profile record.
- **Quick facts grid**: 4 small white cards in a 2×4 grid showing key stats (population,
  heroes/active users, neighborhoods, km²).

These give the user the context they need to interpret the rest of the dashboard.
...
**Then** 4 small cards appear in a row: Habitantes (e.g., "22k"), Heróis (active CityHero users,
brand color), Bairros, km²\
...
**Given** a city wants to swap "Heróis" with a different metric (e.g., "Aprovações" for elected
officials)\
...
The `cities` table has fields for `about_md`, `population`, `neighborhoods_count`, `area_km2`. The
hero count is computed on-demand or cached.
