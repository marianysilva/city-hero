...
The visual layout of the city-select screen: header with back button and
step indicator (1 of 5), title and subtitle, the prominent GPS detection
card, the search input, the list of cities (active + coming soon), and a
footer hint about switching cities later.

This task focuses on the layout and styling only — list contents, GPS
behavior, and selection are implemented by their own tasks.
...
**And** contains a location pin icon, a "Detectamos" label, the city name, GPS precision text, and a primary "Confirmar ✓" CTA button
**And** the actual GPS data is wired by task `04-gps-auto-detect`; the layout itself accepts placeholders or empty state
...
**Then** each row has a flag/emoji on the left, name and subtitle in the middle, status badge on the right
...
**And** each city row is announced with its name, subtitle, and status ("Pôrto Belo SC, pilot, active")
...
`StepIndicator` ("Passo 1 de 5") is **not** a screen-local component —
it's the shared molecule in `@cityhero/design-system` (see
`docs/engineering/component-inventory.md` · Molecules row
`StepIndicator`). Consumed identically by tasks 02, 03, 04, 05, and 24.

### Component behavior

- The screen receives data props from its container/orchestrator: a list of cities, the GPS detection result (nullable), and callbacks for tapping a row and confirming the GPS suggestion.
...
- `CityRow` renders a single city in three visual states: `active`, `selected` (transient when the user confirms), `coming_soon`.
...
The screen background uses a `white → brand.50` vertical gradient. In dark mode, it switches to a tonally equivalent dark gradient.
...
- **No active cities returned**: list shows a friendly empty state ("Estamos chegando em sua cidade em breve") with a CTA to join a waitlist.
...
- **Software keyboard appears for search**: the layout adjusts so the input stays visible above the keyboard.
...
- **Unit**: renders all subcomponents; GPS card is conditionally rendered; rows show the right visual state per status.
