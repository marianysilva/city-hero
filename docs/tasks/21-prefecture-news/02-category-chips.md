...
A horizontal scrollable row of category chips: **Todas · N** (default
active), **🚨 Alertas · N** (highlighted in rose), **🩺 Saúde**, **🚧
Obras**, **🎓 Educação**, **🎉 Eventos**, **🏛️ Transparência**, etc.
Tapping a chip narrows the list below. The chips are sticky — they
stay visible as the user scrolls.
...
- `Alertas` — `icon` 🚨, `count` reflects active alerts; the screen passes a variant hint so `FilterChipRow` can render the rose accent.
- `Saúde` — `icon` 🩺.
- `Obras` — `icon` 🚧.
- `Educação` — `icon` 🎓.
- `Eventos` — `icon` 🎉.
...
For MVP, categories are statically defined (Alertas, Saúde, Obras, Educação, Eventos, Transparência). Future: per-city catalog with admin tooling.
...
- **Many categories**: horizontal scroll with end-of-list indicators (visual gradient hinting more content).
