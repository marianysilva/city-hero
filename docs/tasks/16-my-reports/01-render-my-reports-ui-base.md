...
- "Meus reportes · N" main title), a scrollable area for KPI strip, filter chips, pending offline
  card, the list, and the bridge card (tasks 02–06), plus the persistent bottom nav with "Mais" or
  "Profile" highlighted depending on entry point.
...
- **Title count diverges from list count** (during background sync): the title shows the
  server-confirmed count; the list updates incrementally.
