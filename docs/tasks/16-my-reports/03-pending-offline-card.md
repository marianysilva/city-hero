...
A dashed-border amber card highlighted at the top of the scroll area (below the filter chips) when
there are reports waiting in the offline queue. It reassures the user that nothing was lost: "2
reportes aguardando envio · Serão sincronizados quando tiver rede." A "Ver" button navigates to
SCREEN 18 (Sync Queue) for details.
...
- The card reads from the offline queue's store (per `00-foundation/09`) the current count and
  aggregated state.
...
- **Item count is very high (e.g., 30+ items)**: the card still works; the user can drill into the
  sync screen for management.
