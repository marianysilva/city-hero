...
> **Effort:** S (≤1 day)
> **Dependencies:** `09-manual-report/02-category-grid.md`, `09-manual-report/03-photo-thumbnail.md`, `09-manual-report/04-mini-map-location.md`, `09-manual-report/05-ai-feedback-loop.md`
> **Status:** ⬜ Not started
> **Labels:** `mobile`, `frontend`, `screen`, `ux`

## Context

The CTA at the bottom of the manual report screen. When the user taps
"Continuar →", it gathers the screen state (category, photo, location,
AI feedback flag) and navigates to the standard Confirmação do Reporte
screen (SCREEN 10), where the user reviews and submits to the backend.

This task does not create the report itself — that's owned by SCREEN 10
and the report-create endpoint. Here we hand off the validated payload.
...
**Then** the screen gathers the state: category (primary + optional secondary), photo (URI + flags), location (lat, lng, address), AI-feedback flag, original AI detection (if any), entry context (low_confidence / no_photo / user_opted_out)
...
**And** Confirmação handles the no-photo case (allowed when AI couldn't be used; still a valid report with category + location + description)
...
**Then** it's announced with its enabled/disabled state and the action ("Continue to confirmation, disabled because category is missing")
...
- `useContinueManualReport` aggregates state from the four hooks (`useReportCategory`, `usePhotoState`, `useReportLocation`, `useAiFeedbackConsent`).
- It exposes `canContinue` (boolean) for the CTA gating and `continue()` (function) that gathers and navigates.
- The navigation passes the payload as route params (or via a small in-memory store if the payload is too large for params).
- A back-navigation handler preserves the state in the manual report screen (so re-entering doesn't reset).
...
- `location` — lat, lng, accuracy, source (live/cached), reverse-geocoded address, optional city_warning flag
...
This task doesn't call the backend directly. The Confirmação screen (SCREEN 10) submits the report and triggers the upload pipeline, anonymization, and AI feedback storage.
...
- **Payload too large for nav params**: use an in-memory bridge store keyed by a UUID; Confirmação reads from it.
- **User navigates away mid-flow**: state is preserved in the screen for the session; cleared on app cold start.
- **Backend submission later fails**: handled by Confirmação's flow (retry / queue offline) — not this task's concern.

## Privacy / LGPD

The payload itself is not transmitted yet — only handed to the next screen. The user can still abandon before any data leaves the device.
...
- **Integration**: with all fields populated, continue navigates correctly; with missing fields, CTA disabled.
