...
The visual rendering of the Splash screen. No routing logic or checks —
purely presentation. App logo + name + tagline ("Sua cidade nas suas mãos") +
subtle entrance animation.

It also enforces the **minimum display time** (800ms) to avoid the splash
flickering when initialization checks complete instantly.
...
**And** the name and tagline fade in and translate slightly upward, with a small delay after the logo
...
- The screen accepts a callback (`onReady`) provided by its parent (which is doing the init sequence in task 02). When called, it triggers the navigation hand-off, but only after the minimum display time has elapsed.
- The animated logo runs its entrance animation on mount. It checks the user's reduce-motion preference and skips animation when the preference is on.
- The loading indicator is hidden by default and appears only after a 5s threshold, signaling to the user that something is taking longer than expected.
- A hard timeout at 10s force-completes the splash with a partial-init flag, logged to telemetry for analysis.
...
Use a Reanimated-based animation pipeline: shared values for opacity, scale, and translation; smooth timing curves; cleanup on unmount.
...
- **Fatal error before mount**: handled by the global error boundary (`00-foundation/15-error-boundary.md`).
...
- **Unit**: renders logo, name, and tagline; the minimum display time is enforced even when the parent signals readiness immediately; reduce-motion preference disables animation; the loading indicator surfaces after the configured threshold.
