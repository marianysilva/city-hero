...
> `17-detail-merged/03-comparison-cards.md`\
> **Status:** ⬜ Not started\
> **Labels:** `mobile`, `frontend`, `screen`, `ux`

## Context

The sticky bottom CTA bar with a single primary action: "Ver ticket principal →". Tapping it
navigates the user to the parent ticket's detail (SCREEN 13 if open, SCREEN 14 if resolved). The
button uses an amber gradient that matches the merge banner (task 02) — a deliberate visual
connection between "what happened" and "where to go next".
...
**When** the user taps the CTA\
**Then** navigation still proceeds; the destination screen handles offline state per its own logic
(`13-detail-in-progress/01` covers this)

### Scenario · Real-time parent state change

**Given** the parent's state changes while the user is on this screen (e.g., resolved via
WebSocket)\
...
- Tap callback navigates based on state, using a small router helper
  (`navigateToReportDetail(reportId, state)`).
...
- **Parent's state ambiguous in cache**: refetch the parent summary before navigating; if still
  unclear, fall back to SCREEN 13.
