...
> `24-irregularity-report/02-step-1-program-selection.md`\
> **Status:** ⬜ Not started\
> **Labels:** `mobile`, `frontend`, `backend`, `screen`

## Context

Step 2: the user picks **which authority** they want to file with. Based on the program/area chosen
in step 1, the screen recommends the most relevant authorities first:
...
Each card shows the authority's logo (emoji), name, scope (federal/state/municipal), short
description, and accepted channels (email, web form, in-app).
...
The hook fetches the authorities catalog and computes recommendations based on the program from
step 1.
...
The endpoint returns each authority's metadata + supported channels (email address, web form URL,
etc.).

## Database

A small `authorities` table with: id, name, scope (federal/state/municipal), description, channels
(jsonb), recommended_programs (jsonb).

## Edge Cases

- **No authority matches the program** (very rare): "Outro" fallback to a generic web form for
  Ouvidoria-Geral.
