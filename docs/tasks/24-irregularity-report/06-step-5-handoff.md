...
The final step: the user reviews a summary, then taps "Enviar para {authority}". The screen
generates the pre-formatted message in the authority's expected format and hands off via:
...
After handoff, a confirmation screen shows the action ("✓ Encaminhado para {authority}") and a small
explainer that the authority will respond via its channel.

CityHero **never stores** the complaint content; only telemetry records that a handoff happened (no
content).
...
**Then** a summary card lists: chosen program, authority, identification mode, form preview
(truncated)\
...
**And** the channel + next-steps info ("A {authority} vai responder pelo email cadastrado / via
Portal")\
...
**Then** the user gets a soft fallback: "Não conseguimos abrir o email. Copie o texto e envie pra
{address}."\
...
`composeComplaintMessage` is a small util that takes the program, authority, form data,
identification, and produces the email subject/body (or web-form params).
...
- **Authority's channel changed**: the catalog (per task 03) is consulted on each handoff; stale
  URLs are detected.
- **User's email client missing**: fall back to copy + manual paste.

## Privacy / LGPD

The principle: **CityHero never stores the complaint content.** Analytics only record metadata
(authority, program, identified). The Brazilian channel handles the data subject's rights to
access/erasure of their complaint — CityHero is not a data controller for this content.
