...
The first step of the wizard: the user picks **which program or area** the irregularity concerns.
The list includes social programs (Bolsa Família, BPC, etc.), city services (obras, saúde,
educação), and a generic "Outro" for anything not listed.

If the user came from a specific program detail (e.g., SCREEN 23 Bolsa Família), the program is
**pre-selected** as a convenience.
...
The programs catalog is the same source as SCREEN 22 (`/api/v1/cities/{id}/programs`). No new
endpoint.
...
- **Pre-fill context invalid** (program not in catalog): falls back to "Outro" with the program name
  as the description.
