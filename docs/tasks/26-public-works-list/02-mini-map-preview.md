...
A small map preview at the top of the screen (~140-180dp tall) showing all active works as colored
pins (color per status). The user's location is shown as a pulse. Tapping a pin scrolls the list
below to the corresponding row. Tapping the map area itself toggles to full-screen map mode (via
task 01's toggle).
