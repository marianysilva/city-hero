...
The scrollable list below the chips. Each row card shows: status pill (colored), title (e.g.,
"Recapeamento da Av. Atlântica"), address, dates (start/end or current period), budget (when
public), contractor (when public), and a small progress bar. Tapping opens SCREEN 27 (Detail).
...
`public_works` table with: id, city_id, title, address, category, status, start_date, end_date,
budget, contractor, geo, progress_pct, updated_at, source_url. Indexes on
`(city_id, status, updated_at desc)`.
