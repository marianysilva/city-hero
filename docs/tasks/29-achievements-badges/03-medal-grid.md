...
A 3-column grid of medal cards. Each card: medal emoji (color when unlocked, gray silhouette when
locked) + name + small "Conquistada {date}" or "{progress} de {target}" indicator. Tapping opens the
detail sheet (task 04). Hidden/secret medals show as locked silhouettes with "???" until unlocked.
...
- **Hidden medals not in default filter**: shown only after unlock unless explicitly filtered
  "Conquistadas".
