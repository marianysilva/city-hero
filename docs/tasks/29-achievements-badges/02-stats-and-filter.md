...
A small "12 de 28 conquistadas" stat with a horizontal progress bar at the top, followed by filter
chips: **Todas**, **Conquistadas**, **Em Progresso**, **Bloqueadas**, plus category chips (Reportes,
Comunidade, Cidade, Especiais).
...
The chip row(s) render the shared `FilterChipRow` from `@cityhero/design-system`. The screen owns
the chip definitions and the `onChipPress` callback that filters the data; no styling lives in this
...
- `Reportes`, `Comunidade`, `Cidade`, `Especiais` — multi-select; combined with the completion-state
  row via AND semantics in `useAchievementsFilters`.

The screen's `onChipPress(id)` callback delegates to the store, which updates the active filter set
and triggers the medal grid (task 03) to re-filter.
