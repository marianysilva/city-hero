...
A prominent gradient banner at the top of the screen showing the current connectivity + queue state.
Variants:

- **Offline**: amber→rose gradient, "📶 Sem conexão · 3 reportes na fila · Estão salvos no seu
  celular. Vão sozinhos quando o sinal voltar."
- **Online, items pending**: emerald→teal gradient, "✓ Conectado · sincronizando 3 reportes".
- **Online, queue empty**: hidden (or a small "Tudo certo" banner).
- **Sync errored**: rose gradient with "Algumas tentativas falharam · toque pra ver".

The banner is the screen's emotional core — reassuring when offline, celebratory when sync
completes.
...
A small state machine maps `(connectivity, queue_summary)` to one of: `offline_with_items`,
`online_syncing`, `online_done`, `error`.
...
- **All items failed permanently**: the error variant persists until the user discards or retries
  them.
