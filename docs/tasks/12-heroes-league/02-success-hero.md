...
The emerald gradient hero at the top of the screen. Centered: a white circular checkmark badge, the
protocol label ("PROTOCOLO #2847 · ENVIADO"), the headline ("Ótimo trabalho, herói!"), and a
celebration pill showing the XP and any unlocked medal ("+50 XP · 🏅 Olho Vivo desbloqueada"). Small
SVG confetti and sparkles decorate the gradient to amplify the celebration without overwhelming.

This is the moment of payoff for the user — the more emotionally resonant this lands, the more
likely they'll share.
...
**And** the celebration adds a subtle level-up animation (e.g., a soft glow ring around the medal
pill)\
...
**When** the hero renders\
**Then** the copy is in English ("Great work, hero!", "PROTOCOL #2847 · SENT", "+50 XP · 🏅 Sharp
Eye unlocked")
...
- `SuccessHero` receives the protocol, XP granted, medal info, and a flag for first-report. It
  manages mount animations.
- `ConfettiBackground` is an SVG component with multiple shapes (circles, small triangles) at
  varying opacities and positions. It honors reduced motion.
- `XpMedalPill` is reused from `10-report-confirm/07-xp-medal-preview.md` (shared component in
...
The hero is rendered once on mount and doesn't update reactively (the data is stable for the
screen's lifetime).

## Backend

Not applicable to this task. The data is read from the screen's navigation params (set by
`10-report-confirm/08`).
...
- **Protocol not yet assigned**: rare timing issue; the placeholder "Protocolo · gerando…" is shown
  until assigned.
