...
A short educational block explaining what's public (per STF MS 36.020/2020) and what's protected
(LGPD), with links to the relevant laws. Below it, a primary CTA "🛡️ Denunciar irregularidade" sits
in the sticky bottom bar (per task 01's slot), pre-filling the report with the program context when
the user navigates to SCREEN 24.
...
**And** a short paragraph: "O cadastro do Bolsa Família é público (STF MS 36.020/2020). Os valores
individuais são protegidos pela LGPD. Mostramos só dados agregados."\
...
The notes themselves are a transparency device — they explain to the user what's happening with
data.
