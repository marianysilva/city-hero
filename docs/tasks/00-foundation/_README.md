...
| #   | Task                                                                                           | Effort | Blocks                        |
| --- | ---------------------------------------------------------------------------------------------- | ------ | ----------------------------- |
| 01  | [Monorepo setup (npm workspaces + Turborepo, ESLint, Prettier, Husky)](./01-monorepo-setup.md) | M      | everything                    |
| 02  | [Design tokens · colors, typography, spacing, shadows](./02-design-tokens.md)                  | M      | all UI                        |
...
- **05 (API client) and 06 (auth system) reference each other's endpoints/shapes** but the build
  order is one-directional — 05 first, then 06 — since only two of 05's scenarios (token refresh,
  single-flight refresh) are actually blocked on 06 shipping `/auth/refresh`. See the Context note
  in each file.
- **Open design-decision flags from the 2026-07-15 doc-alignment pass**, not yet resolved: 09's use
  of WatermelonDB may be heavier than this app needs for a simple FIFO retry queue (see its
  Context); 10's premise of using Leaflet (a web-first library) inside React Native may be a worse
  fit than a native map library like `react-native-maps` or MapLibre (see its Context).
