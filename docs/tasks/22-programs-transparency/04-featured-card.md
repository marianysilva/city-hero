...
A prominent gradient (sky → teal) hero card highlighting the **featured program** of the city (Bolsa
Família for the Pôrto Belo MVP). The card includes: governance + theme pills ("FEDERAL · SOCIAL"), a
"⭐ EM DESTAQUE" pill, the program emoji + name + description, and a 3-cell KPI grid (famílias,
valor mensal, índice de pobreza vs comparável). Tapping opens SCREEN 23 (Bolsa Família detail).

The "featured" mechanic is configurable per city — different cities spotlight different programs.
...
The endpoint returns the program's identifier + key metrics. The detail screen does its own deep
fetch.
