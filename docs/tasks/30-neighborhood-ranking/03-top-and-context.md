...
> **Effort:** M (1-2 days)
> **Dependencies:** `30-neighborhood-ranking/01-render-ranking-ui-base.md`, `00-foundation/05-api-client.md`
> **Status:** ⬜ Not started
> **Labels:** `mobile`, `backend`, `screen`

## Context

The main leaderboard structure: a podium card highlighting top 3 with bigger visuals + badges, then a list of positions 4-10. Below: a divider + "Seu pelotão" section showing the user's rank with 2-3 positions above and below (so the user knows what's just ahead).

If the user is not on the top 10, their rank is shown separately in "Seu pelotão". If they're in the top 10, the contextual rank section is hidden.
...
**Then** top 3 appear on a podium card (1st with crown, 2nd silver, 3rd bronze) with avatar + name + XP
...
**Then** the other user's mini-profile sheet opens (a small public-safe view: avatar, name, level, medals count) — out of MVP if needed; for MVP, no-op with informational hover
...
Computed from `users.xp` aggregated over the period. For monthly periods, a `monthly_xp_snapshots` table avoids recomputing.
