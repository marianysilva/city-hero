...
A presentational row used in the leaderboard and pelotão sections. Each row: position number, avatar
(initial or 🥷), name, level title, XP value (with the period's framing), and a delta indicator (↑ N
positions / ↓ N positions / unchanged) compared to the previous period.
