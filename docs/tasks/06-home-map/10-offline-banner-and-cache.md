...
> `00-foundation/09-offline-queue.md`, `01-splash/05-cold-start-offline.md`\
> **Status:** ⬜ Not started\
> **Labels:** `mobile`, `frontend`, `screen`, `offline`, `resilience`

## Context

When the device is offline, Home should remain useful: cached pins (from the last successful fetch)
stay visible, the offline banner clearly communicates the state, and any pending offline-queue items
are discoverable from the same banner.
...
**And** if the cache is older than a threshold (e.g., 24h), the banner adds "Dados podem estar
desatualizados"
...
**Then** the banner reflects the queue count when there are pending items ("3 reportes aguardando
sync")\
...
TanStack Query is configured to persist its cache to disk (per the API client's React Query setup,
which itself is part of foundation 05 hygiene). The home reports query reads from cache first, then
refetches from network when online.

### Connectivity listener

A small reusable hook (`useConnectivity`) wraps the platform's connectivity API and returns the
state. Used here and in other screens.
...
- **Cache contains pins from a different city** (rare; user switched cities): scope cache by
  `city_id` so the wrong city's pins never appear.
- **Banner overlaps the map and obscures pins**: keep the banner thin (≤32dp).

## Privacy / LGPD

The cached reports include anonymized photo URLs. The cache lives within the app sandbox, protected
by OS-level encryption. On logout, the cache is cleared.
...
- **Integration**: cold-start offline with cached pins; warm-start losing connectivity; reconnect
  drains queue.
